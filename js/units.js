//units.js
var corrinF = {
    n: "Corrin (F)",
    vName: "corrinF",
    baseClass: ["Nohr Princess"],
    bff: [],
    sRank: ["Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Arthur", "Odin", "Niles", "Laslow", "Benny", "Leo", "Keaton", "Xander", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Shigure", "Dwyer", "Siegbert", "Forrest", "Ignatius", "Percy", "Shiro", "Kiragi", "Asugi", "Hisame", "Rhajat"],
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    baseHPGR: 45,
    baseStrGR: 45,
    baseMagGR: 30,
    baseSklGR: 40,
    baseSpdGR: 45,
    baseLckGR: 45,
    baseDefGR: 35,
    baseResGR: 25,
    hpGR: 45,
    strGR: 45,
    magGR: 30,
    sklGR: 40,
    spdGR: 45,
    lckGR: 45,
    defGR: 35,
    resGR: 25,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 1, 0],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 1],
    isChild: false,
    isRoyal: true,
    sex: "F"
}
var corrinM = {
    n: "Corrin (M)",
    vName: "corrinM",
    baseClass: ["Nohr Prince"],
    bff: [],
    sRank: ["Felicia", "Azura", "Mozu", "Elise", "Effie", "Niles", "Nyx", "Camilla", "Selena", "Beruka", "Peri", "Charlotte", "Flora", "Rinkah", "Sakura", "Hana", "Orochi", "Hinoka", "Setsuna", "Oboro", "Kagero", "Reina", "Scarlet", "Anna", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Selkie", "Mitama", "Caeldori", "Rhajat"],
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    baseHPGR: 45,
    baseStrGR: 45,
    baseMagGR: 30,
    baseSklGR: 40,
    baseSpdGR: 45,
    baseLckGR: 45,
    baseDefGR: 35,
    baseResGR: 25,
    hpGR: 45,
    strGR: 45,
    magGR: 30,
    sklGR: 40,
    spdGR: 45,
    lckGR: 45,
    defGR: 35,
    resGR: 25,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 1, 0],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 1],
    isChild: false,
    isRoyal: true,
    sex: "M"
}
var gunter = {
    n: "Gunter",
    vName: "gunter",
    baseClass: ["Cavalier", "Mercenary", "Wyvern Rider"],
    bff: ["Jakob"],
    sRank: ["Corrin (F)"],
    strMod: 2,
    magMod: 0,
    sklMod: 1,
    spdMod: -2,
    lckMod: 0,
    defMod: 2,
    resMod: -2,
    hpGR: 15,
    strGR: 5,
    magGR: 0,
    sklGR: 5,
    spdGR: 0,
    lckGR: 15,
    defGR: 5,
    resGR: 5,
    cpb: [0, 0, 0, 0, 0, 1, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 1, 0, 0, 0, 0],
    spb: [1, 0, 0, 0, 0, 1, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var felicia = {
    n: "Felicia",
    vName: "felicia",
    baseClass: ["Troubadour", "Mercenary"],
    bff: ["Hana", "Peri", "Flora"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Xander", "Leo", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Ryoma", "Takumi", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato"],
    strMod: -2,
    magMod: 2,
    sklMod: 0,
    spdMod: 1,
    lckMod: 0,
    defMod: -1,
    resMod: 1,
    hpGR: 40,
    strGR: 10,
    magGR: 35,
    sklGR: 30,
    spdGR: 40,
    lckGR: 55,
    defGR: 15,
    resGR: 35,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 1],
    apb: [0, 1, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 1, 0, 0, 1],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var jakob = {
    n: "Jakob",
    vName: "jakob",
    baseClass: ["Troubadour", "Cavalier"],
    bff: ["Gunter", "Silas", "Takumi"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Camilla", "Elise", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Hinoka", "Sakura", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro"],
    strMod: 2,
    magMod: -2,
    sklMod: 2,
    spdMod: 0,
    lckMod: -1,
    defMod: 0,
    resMod: -1,
    hpGR: 50,
    strGR: 30,
    magGR: 15,
    sklGR: 40,
    spdGR: 35,
    lckGR: 45,
    defGR: 25,
    resGR: 25,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [0, 0, 1, 0, 0, 0, 0],
    spb: [1, 0, 1, 0, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var kaze = {
    n: "Kaze",
    vName: "kaze",
    baseClass: ["Ninja", "Samurai"],
    bff: ["Saizo", "Silas", "Xander"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Camilla", "Elise", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Hinoka", "Sakura", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro"],
    strMod: -2,
    magMod: 0,
    sklMod: 2,
    spdMod: 3,
    lckMod: -2,
    defMod: -1,
    resMod: 1,
    hpGR: 55,
    strGR: 40,
    magGR: 0,
    sklGR: 45,
    spdGR: 65,
    lckGR: 20,
    defGR: 20,
    resGR: 35,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 1, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 1],
    spb: [0, 0, 1, 1, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var azura = {
    n: "Azura",
    vName: "azura",
    baseClass: ["Songstress", "Sky Knight"],
    bff: ["Hinoka", "Sakura", "Elise"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Xander", "Leo", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Ryoma", "Takumi", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato"],
    strMod: 0,
    magMod: 0,
    sklMod: 1,
    spdMod: 3,
    lckMod: 0,
    defMod: -3,
    resMod: 0,
    hpGR: 25,
    strGR: 50,
    magGR: 25,
    sklGR: 60,
    spdGR: 60,
    lckGR: 40,
    defGR: 15,
    resGR: 35,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 1, 0, 0, 0, 0],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [0, 0, 0, 1, 1, 0, 0],
    isChild: false,
    isRoyal: true,
    sex: "F"
}
var silas = {
    n: "Silas",
    vName: "silas",
    baseClass: ["Cavalier", "Mercenary"],
    bff: ["Ryoma", "Jakob", "Kaze"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Camilla", "Elise", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Hinoka", "Sakura", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro"],
    strMod: 1,
    magMod: 0,
    sklMod: 2,
    spdMod: 0,
    lckMod: -1,
    defMod: 0,
    resMod: -1,
    hpGR: 40,
    strGR: 45,
    magGR: 5,
    sklGR: 50,
    spdGR: 40,
    lckGR: 40,
    defGR: 40,
    resGR: 25,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 1, 0, 0, 0, 0],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [1, 0, 0, 0, 0, 1, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var shura = {
    n: "Shura",
    vName: "shura",
    baseClass: ["Outlaw", "Ninja", "Fighter"],
    sRank: ["Corrin (F)"],
    bff: [],
    strMod: -1,
    magMod: 0,
    sklMod: -1,
    spdMod: 3,
    lckMod: -1,
    defMod: -2,
    resMod: 2,
    hpGR: 30,
    strGR: 25,
    magGR: 10,
    sklGR: 20,
    spdGR: 35,
    lckGR: 30,
    defGR: 15,
    resGR: 35,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 1],
    apb: [0, 0, 0, 0, 0, 0, 1],
    spb: [0, 0, 0, 1, 0, 0, 1],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var izana = {
    n: "Izana",
    vName: "izana",
    baseClass: ["Monk", "Samurai", "Apothecary"],
    sRank: ["Corrin (F)"],
    bff: [],
    strMod: 0,
    magMod: 1,
    sklMod: 1,
    spdMod: -2,
    lckMod: 0,
    defMod: 0,
    resMod: 1,
    hpGR: 45,
    strGR: 15,
    magGR: 35,
    sklGR: 55,
    spdGR: 30,
    lckGR: 45,
    defGR: 35,
    resGR: 35,
    cpb: [0, 0, 1, 0, 0, 0, 0],
    bpb: [0, 1, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 1, 0],
    spb: [0, 0, 1, 0, 0, 0, 1],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var mozu = {
    n: "Mozu",
    vName: "mozu",
    baseClass: ["Villager", "Archer"],
    bff: ["Oboro", "Effie", "Nyx"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Xander", "Leo", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Ryoma", "Takumi", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato"],
    strMod: 0,
    magMod: 0,
    sklMod: 1,
    spdMod: 1,
    lckMod: 1,
    defMod: 0,
    resMod: -2,
    hpGR: 30,
    strGR: 40,
    magGR: 5,
    sklGR: 50,
    spdGR: 55,
    lckGR: 45,
    defGR: 35,
    resGR: 30,
    cpb: [0, 0, 1, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 1, 0, 0],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [0, 0, 1, 0, 1, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var elise = {
    n: "Elise",
    vName: "elise",
    baseClass: ["Troubadour", "Wyvern Rider"],
    bff: ["Camilla", "Azura", "Effie", "Sakura"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Ryoma", "Takumi"],
    strMod: -1,
    magMod: 3,
    sklMod: -2,
    spdMod: 1,
    lckMod: 1,
    defMod: -3,
    resMod: 1,
    hpGR: 30,
    strGR: 5,
    magGR: 65,
    sklGR: 25,
    spdGR: 55,
    lckGR: 70,
    defGR: 15,
    resGR: 40,
    cpb: [0, 1, 0, 0, 0, 0, 0],
    bpb: [0, 1, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 1, 0, 0],
    spb: [0, 1, 0, 1, 0, 0, 0],
    isChild: false,
    isRoyal: true,
    sex: "F"
}
var arthur = {
    n: "Arthur",
    vName: "arthur",
    baseClass: ["Fighter", "Cavalier"],
    bff: ["Benny", "Keaton", "Niles", "Azama"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Camilla", "Elise", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Kagero", "Setsuna"],
    strMod: 1,
    magMod: 0,
    sklMod: 3,
    spdMod: 0,
    lckMod: -3,
    defMod: 1,
    resMod: -1,
    hpGR: 50,
    strGR: 45,
    magGR: 0,
    sklGR: 55,
    spdGR: 35,
    lckGR: 5,
    defGR: 45,
    resGR: 20,
    cpb: [0, 0, 1, 0, 0, 0, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [1, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 2, 0, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var effie = {
    n: "Effie",
    vName: "effie",
    baseClass: ["Knight", "Troubadour"],
    bff: ["Elise", "Mozu", "Nyx", "Hana"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Xander", "Leo", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Hayato", "Azama"],
    strMod: 3,
    magMod: 0,
    sklMod: -1,
    spdMod: 1,
    lckMod: 0,
    defMod: -1,
    resMod: -1,
    hpGR: 35,
    strGR: 60,
    magGR: 0,
    sklGR: 35,
    spdGR: 50,
    lckGR: 50,
    defGR: 35,
    resGR: 30,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 1, 0, 0],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [2, 0, 0, 0, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var odin = {
    n: "Odin",
    vName: "odin",
    baseClass: ["Dark Mage", "Samurai"],
    bff: ["Leo", "Laslow", "Niles", "Hinata"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Camilla", "Elise", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Orochi", "Kagero"],
    strMod: 0,
    magMod: 1,
    sklMod: 1,
    spdMod: -1,
    lckMod: 1,
    defMod: 0,
    resMod: -1,
    hpGR: 55,
    strGR: 35,
    magGR: 30,
    sklGR: 55,
    spdGR: 35,
    lckGR: 60,
    defGR: 40,
    resGR: 20,
    cpb: [0, 1, 0, 0, 0, 0, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 1, 0, 0],
    spb: [0, 1, 1, 0, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var niles = {
    n: "Niles",
    vName: "niles",
    baseClass: ["Outlaw", "Dark Mage"],
    bff: ["Leo", "Odin", "Arthur", "Subaki"],
    sRank: ["Corrin (F)", "Corrin (M)", "Felicia", "Azura", "Mozu", "Camilla", "Elise", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Setsuna", "Oboro"],
    strMod: -2,
    magMod: 0,
    sklMod: -1,
    spdMod: 3,
    lckMod: 0,
    defMod: 0,
    resMod: 1,
    hpGR: 40,
    strGR: 35,
    magGR: 20,
    sklGR: 40,
    spdGR: 50,
    lckGR: 30,
    defGR: 30,
    resGR: 40,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 1],
    spb: [0, 1, 0, 0, 0, 0, 1],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var nyx = {
    n: "Nyx",
    vName: "nyx",
    baseClass: ["Dark Mage", "Outlaw"],
    bff: ["Mozu", "Charlotte", "Effie", "Orochi"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Xander", "Leo", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Hayato", "Subaki"],
    strMod: 0,
    magMod: 3,
    sklMod: -2,
    spdMod: 2,
    lckMod: -1,
    defMod: -2,
    resMod: 1,
    hpGR: 30,
    strGR: 5,
    magGR: 50,
    sklGR: 35,
    spdGR: 50,
    lckGR: 20,
    defGR: 15,
    resGR: 30,
    cpb: [0, 1, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [0, 1, 0, 0, 0, 0, 0],
    spb: [0, 1, 0, 1, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var camilla = {
    n: "Camilla",
    vName: "camilla",
    baseClass: ["Wyvern Rider", "Dark Mage"],
    bff: ["Elise", "Beruka", "Selena", "Hinoka"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Ryoma", "Takumi"],
    strMod: 1,
    magMod: -1,
    sklMod: 1,
    spdMod: 1,
    lckMod: -2,
    defMod: 1,
    resMod: 0,
    hpGR: 40,
    strGR: 50,
    magGR: 25,
    sklGR: 50,
    spdGR: 55,
    lckGR: 25,
    defGR: 35,
    resGR: 45,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 1],
    apb: [1, 0, 0, 0, 0, 0, 0],
    spb: [1, 0, 0, 0, 0, 1, 0],
    isChild: false,
    isRoyal: true,
    sex: "F"
}
var selena = {
    n: "Selena",
    vName: "selena",
    baseClass: ["Mercenary", "Sky Knight"],
    bff: ["Camilla", "Beruka", "Peri", "Setsuna"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Xander", "Leo", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Subaki", "Hinata"],
    strMod: -1,
    magMod: 0,
    sklMod: -1,
    spdMod: 2,
    lckMod: 0,
    defMod: 1,
    resMod: 0,
    hpGR: 40,
    strGR: 30,
    magGR: 5,
    sklGR: 25,
    spdGR: 45,
    lckGR: 30,
    defGR: 45,
    resGR: 30,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 1, 0],
    apb: [0, 0, 1, 0, 0, 0, 0],
    spb: [1, 0, 0, 1, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var beruka = {
    n: "Beruka",
    vName: "beruka",
    baseClass: ["Wyvern Rider", "Fighter"],
    bff: ["Camilla", "Charlotte", "Selena", "Oboro"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Xander", "Leo", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Azama", "Saizo"],
    strMod: -1,
    magMod: 0,
    sklMod: 2,
    spdMod: -2,
    lckMod: 0,
    defMod: 2,
    resMod: -1,
    hpGR: 45,
    strGR: 30,
    magGR: 10,
    sklGR: 55,
    spdGR: 30,
    lckGR: 45,
    defGR: 40,
    resGR: 25,
    cpb: [0, 0, 1, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 1, 0],
    apb: [0, 0, 1, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 1, 1, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var laslow = {
    n: "Laslow",
    vName: "laslow",
    baseClass: ["Mercenary", "Ninja"],
    bff: ["Xander", "Odin", "Keaton", "Saizo"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Camilla", "Elise", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Orochi", "Hana"],
    strMod: 1,
    magMod: 0,
    sklMod: 2,
    spdMod: -1,
    lckMod: 1,
    defMod: -1,
    resMod: -1,
    hpGR: 50,
    strGR: 45,
    magGR: 0,
    sklGR: 45,
    spdGR: 30,
    lckGR: 55,
    defGR: 35,
    resGR: 25,
    cpb: [0, 0, 1, 0, 0, 0, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 1, 0, 0],
    spb: [1, 0, 0, 0, 1, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var peri = {
    n: "Peri",
    vName: "peri",
    baseClass: ["Cavalier", "Dark Mage"],
    bff: ["Felicia", "Charlotte", "Selena", "Kagero"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Xander", "Leo", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Hinata", "Kaden"],
    strMod: 1,
    magMod: 0,
    sklMod: -1,
    spdMod: 1,
    lckMod: 0,
    defMod: -2,
    resMod: 2,
    hpGR: 30,
    strGR: 50,
    magGR: 5,
    sklGR: 30,
    spdGR: 50,
    lckGR: 35,
    defGR: 25,
    resGR: 45,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 1],
    spb: [1, 0, 0, 1, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var benny = {
    n: "Benny",
    vName: "benny",
    baseClass: ["Knight", "Fighter"],
    bff: ["Keaton", "Arthur", "Hayato"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Camilla", "Elise", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Rinkah", "Oboro"],
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: -3,
    lckMod: 0,
    defMod: 3,
    resMod: 1,
    hpGR: 50,
    strGR: 40,
    magGR: 0,
    sklGR: 50,
    spdGR: 10,
    lckGR: 35,
    defGR: 55,
    resGR: 45,
    cpb: [0, 0, 0, 0, 0, 1, 0],
    bpb: [0, 0, 1, 0, 0, 0, 0],
    apb: [1, 0, 0, 0, 0, 0, 0],
    spb: [1, 0, 0, 0, 0, 1, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var charlotte = {
    n: "Charlotte",
    vName: "charlotte",
    baseClass: ["Fighter", "Troubadour"],
    bff: ["Peri", "Beruka", "Nyx", "Rinkah"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Xander", "Leo", "Benny", "Keaton", "Arthur", "Odin", "Laslow", "Niles", "Saizo", "Kaden"],
    strMod: 3,
    magMod: 0,
    sklMod: 0,
    spdMod: 2,
    lckMod: 0,
    defMod: -2,
    resMod: -2,
    hpGR: 65,
    strGR: 55,
    magGR: 0,
    sklGR: 35,
    spdGR: 50,
    lckGR: 45,
    defGR: 20,
    resGR: 5,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [1, 0, 0, 1, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var leo = {
    n: "Leo",
    vName: "leo",
    baseClass: ["Dark Mage", "Troubadour"],
    bff: ["Xander", "Odin", "Niles", "Takumi"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Hinoka", "Sakura"],
    strMod: -2,
    magMod: 2,
    sklMod: 0,
    spdMod: -2,
    lckMod: 0,
    defMod: 0,
    resMod: 2,
    hpGR: 45,
    strGR: 25,
    magGR: 55,
    sklGR: 35,
    spdGR: 45,
    lckGR: 45,
    defGR: 30,
    resGR: 45,
    cpb: [0, 1, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 1],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [0, 1, 0, 0, 1, 0, 0],
    isChild: false,
    isRoyal: true,
    sex: "M"
}
var keaton = {
    n: "Keaton",
    vName: "keaton",
    baseClass: ["Wolfskin", "Fighter"],
    bff: ["Benny", "Arthur", "Laslow", "Kaden"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Camilla", "Elise", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Rinkah", "Hana"],
    strMod: 3,
    magMod: 0,
    sklMod: -2,
    spdMod: -1,
    lckMod: 0,
    defMod: 2,
    resMod: -1,
    hpGR: 60,
    strGR: 60,
    magGR: 0,
    sklGR: 20,
    spdGR: 35,
    lckGR: 30,
    defGR: 50,
    resGR: 25,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 1, 0],
    spb: [1, 0, 0, 0, 0, 1, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var xander = {
    n: "Xander",
    vName: "xander",
    baseClass: ["Cavalier", "Wyvern Rider"],
    bff: ["Kaze", "Laslow", "Leo", "Ryoma"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Charlotte", "Effie", "Peri", "Beruka", "Selena", "Nyx", "Hinoka", "Sakura"],
    strMod: 2,
    magMod: -1,
    sklMod: -1,
    spdMod: -1,
    lckMod: 2,
    defMod: 1,
    resMod: -2,
    hpGR: 45,
    strGR: 50,
    magGR: 5,
    sklGR: 40,
    spdGR: 35,
    lckGR: 60,
    defGR: 40,
    resGR: 15,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 1, 0, 0],
    apb: [0, 0, 0, 0, 0, 1, 0],
    spb: [1, 0, 0, 0, 1, 0, 0],
    isChild: false,
    isRoyal: true,
    sex: "M"
}
var flora = {
    n: "Flora",
    vName: "flora",
    baseClass: ["Troubadour", "Dark Mage", "Mercenary"],
    bff: ["Felicia"],
    sRank: ["Corrin (M)"],
    strMod: 1,
    magMod: -1,
    sklMod: 2,
    spdMod: 0,
    lckMod: -1,
    defMod: 1,
    resMod: -1,
    hpGR: 35,
    strGR: 40,
    magGR: 20,
    sklGR: 45,
    spdGR: 30,
    lckGR: 35,
    defGR: 30,
    resGR: 30,
    cpb: [0, 0, 1, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 1, 0],
    apb: [0, 0, 0, 0, 0, 0, 1],
    spb: [1, 0, 0, 0, 1, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var rinkah = {
    n: "Rinkah",
    vName: "rinkah",
    baseClass: ["Oni Savage", "Ninja"],
    bff: ["Orochi", "Kagero", "Oboro", "Charlotte"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Ryoma", "Takumi", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato", "Benny", "Keaton"],
    strMod: -1,
    magMod: 0,
    sklMod: -2,
    spdMod: 1,
    lckMod: 0,
    defMod: 2,
    resMod: 0,
    hpGR: 20,
    strGR: 25,
    magGR: 15,
    sklGR: 50,
    spdGR: 45,
    lckGR: 35,
    defGR: 45,
    resGR: 20,
    cpb: [0, 0, 0, 0, 0, 1, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [1, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 1, 0, 1, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var sakura = {
    n: "Sakura",
    vName: "sakura",
    baseClass: ["Shrine Maiden", "Sky Knight"],
    bff: ["Hinoka", "Hana", "Azura", "Elise"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato", "Xander", "Leo"],
    strMod: 0,
    magMod: 2,
    sklMod: -1,
    spdMod: 1,
    lckMod: 0,
    defMod: -1,
    resMod: 0,
    hpGR: 45,
    strGR: 30,
    magGR: 50,
    sklGR: 40,
    spdGR: 40,
    lckGR: 55,
    defGR: 30,
    resGR: 20,
    cpb: [0, 1, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [0, 0, 0, 0, 1, 0, 0],
    spb: [0, 0, 0, 1, 0, 0, 1],
    isChild: false,
    isRoyal: true,
    sex: "F"
}
var hana = {
    n: "Hana",
    vName: "hana",
    baseClass: ["Samurai", "Shrine Maiden"],
    bff: ["Felicia", "Sakura", "Setsuna", "Effie"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Ryoma", "Takumi", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato", "Laslow", "Keaton"],
    strMod: 1,
    magMod: 0,
    sklMod: 1,
    spdMod: 2,
    lckMod: -1,
    defMod: -3,
    resMod: 1,
    hpGR: 25,
    strGR: 55,
    magGR: 10,
    sklGR: 45,
    spdGR: 55,
    lckGR: 25,
    defGR: 20,
    resGR: 30,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 1, 0, 0, 0, 0],
    spb: [1, 0, 0, 1, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"

}
var subaki = {
    n: "Subaki",
    vName: "subaki",
    baseClass: ["Sky Knight", "Samurai"],
    bff: ["Azama", "Saizo", "Hinata", "Niles"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Hinoka", "Sakura", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro", "Selena", "Nyx"],
    strMod: -1,
    magMod: 0,
    sklMod: 2,
    spdMod: -2,
    lckMod: -1,
    defMod: 3,
    resMod: -1,
    hpGR: 55,
    strGR: 30,
    magGR: 20,
    sklGR: 50,
    spdGR: 20,
    lckGR: 25,
    defGR: 45,
    resGR: 5,
    cpb: [0, 0, 0, 0, 0, 1, 0],
    bpb: [0, 0, 1, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 1, 0],
    spb: [0, 0, 1, 0, 0, 0, 1],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var saizo = {
    n: "Saizo",
    vName: "saizo",
    baseClass: ["Ninja", "Samurai"],
    bff: ["Ryoma", "Subaki", "Kaze", "Laslow"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Hinoka", "Sakura", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro", "Charlotte", "Beruka"],
    strMod: 1,
    magMod: 0,
    sklMod: 3,
    spdMod: -2,
    lckMod: 0,
    defMod: 1,
    resMod: -2,
    hpGR: 40,
    strGR: 50,
    magGR: 45,
    sklGR: 60,
    spdGR: 30,
    lckGR: 55,
    defGR: 45,
    resGR: 10,
    cpb: [0, 0, 0, 0, 1, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [0, 0, 1, 0, 0, 0, 0],
    spb: [1, 0, 0, 0, 1, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var orochi = {
    n: "Orochi",
    vName: "orochi",
    baseClass: ["Diviner", "Apothecary"],
    bff: ["Rinkah", "Kagero", "Oboro", "Nyx"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Ryoma", "Takumi", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato", "Laslow", "Odin"],
    strMod: 0,
    magMod: 3,
    sklMod: 2,
    spdMod: -2,
    lckMod: -1,
    defMod: -2,
    resMod: 1,
    hpGR: 35,
    strGR: 5,
    magGR: 65,
    sklGR: 50,
    spdGR: 15,
    lckGR: 35,
    defGR: 25,
    resGR: 45,
    cpb: [0, 1, 0, 0, 0, 0, 0],
    bpb: [0, 0, 1, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 1],
    spb: [0, 1, 0, 0, 0, 0, 1],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var hinoka = {
    n: "Hinoka",
    vName: "hinoka",
    baseClass: ["Sky Knight", "Spear Fighter"],
    bff: ["Sakura", "Setsuna", "Azura", "Camilla"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato", "Xander", "Leo"],
    strMod: 1,
    magMod: -1,
    sklMod: -1,
    spdMod: 1,
    lckMod: 0,
    defMod: -1,
    resMod: 2,
    hpGR: 45,
    strGR: 45,
    magGR: 15,
    sklGR: 40,
    spdGR: 45,
    lckGR: 40,
    defGR: 35,
    resGR: 40,
    cpb: [0, 0, 0, 0, 0, 0, 1],
    bpb: [0, 0, 0, 0, 1, 0, 0],
    apb: [1, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 1, 0, 0, 1],
    isChild: false,
    isRoyal: true,
    sex: "F"
}
var azama = {
    n: "Azama",
    vName: "azama",
    baseClass: ["Monk", "Apothecary"],
    bff: ["Kaden", "Subaki", "Hayato", "Arthur"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Hinoka", "Sakura", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro", "Effie", "Beruka"],
    strMod: 2,
    magMod: -3,
    sklMod: 0,
    spdMod: 1,
    lckMod: 0,
    defMod: 1,
    resMod: 0,
    hpGR: 55,
    strGR: 50,
    magGR: 20,
    sklGR: 40,
    spdGR: 45,
    lckGR: 40,
    defGR: 40,
    resGR: 20,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 1, 0],
    spb: [1, 0, 0, 0, 1, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var setsuna = {
    n: "Setsuna",
    vName: "setsuna",
    baseClass: ["Archer", "Ninja"],
    bff: ["Hinoka", "Kagero", "Hana", "Selena"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Ryoma", "Takumi", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato", "Niles", "Arthur"],
    strMod: 0,
    magMod: 0,
    sklMod: 1,
    spdMod: 3,
    lckMod: -1,
    defMod: -1,
    resMod: -1,
    hpGR: 30,
    strGR: 20,
    magGR: 0,
    sklGR: 30,
    spdGR: 60,
    lckGR: 30,
    defGR: 15,
    resGR: 40,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 1],
    spb: [0, 0, 1, 1, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var hayato = {
    n: "Hayato",
    vName: "hayato",
    baseClass: ["Diviner", "Oni Savage"],
    bff: ["Kaden", "Azama", "Benny", "Fuga"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Hinoka", "Sakura", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro", "Effie", "Nyx"],
    strMod: 0,
    magMod: 1,
    sklMod: -1,
    spdMod: 2,
    lckMod: 1,
    defMod: -1,
    resMod: -1,
    hpGR: 50,
    strGR: 30,
    magGR: 40,
    sklGR: 30,
    spdGR: 45,
    lckGR: 60,
    defGR: 40,
    resGR: 20,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 0, 0, 1, 0, 0],
    apb: [0, 1, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 1, 1, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var oboro = {
    n: "Oboro",
    vName: "oboro",
    baseClass: ["Spear Fighter", "Apothecary"],
    bff: ["Orochi", "Rinkah", "Mozu", "Beruka"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Ryoma", "Takumi", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato", "Benny", "Niles"],
    strMod: 1,
    magMod: -1,
    sklMod: 1,
    spdMod: 1,
    lckMod: -1,
    defMod: 1,
    resMod: -1,
    hpGR: 30,
    strGR: 40,
    magGR: 20,
    sklGR: 40,
    spdGR: 40,
    lckGR: 40,
    defGR: 40,
    resGR: 30,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 1, 0],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [0, 0, 1, 0, 1, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var hinata = {
    n: "Hinata",
    vName: "hinata",
    baseClass: ["Samurai", "Oni Savage"],
    bff: ["Takumi", "Kaden", "Subaki", "Odin"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Hinoka", "Sakura", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro", "Peri", "Selena"],
    strMod: 1,
    magMod: 0,
    sklMod: -1,
    spdMod: -2,
    lckMod: 0,
    defMod: 2,
    resMod: 0,
    hpGR: 55,
    strGR: 35,
    magGR: 0,
    sklGR: 25,
    spdGR: 15,
    lckGR: 45,
    defGR: 45,
    resGR: 15,
    cpb: [0, 0, 0, 0, 0, 1, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 1, 0],
    spb: [1, 0, 0, 0, 1, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var takumi = {
    n: "Takumi",
    vName: "takumi",
    baseClass: ["Archer", "Spear Fighter"],
    bff: ["Ryoma", "Hinata", "Jakob", "Leo"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro", "Camilla", "Elise"],
    strMod: 1,
    magMod: 0,
    sklMod: 3,
    spdMod: -2,
    lckMod: 1,
    defMod: 0,
    resMod: -2,
    hpGR: 50,
    strGR: 35,
    magGR: 0,
    sklGR: 60,
    spdGR: 40,
    lckGR: 45,
    defGR: 35,
    resGR: 20,
    cpb: [0, 0, 1, 0, 0, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [1, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 1, 0, 0, 1, 0],
    isChild: false,
    isRoyal: true,
    sex: "M"
}
var kagero = {
    n: "Kagero",
    vName: "kagero",
    baseClass: ["Ninja", "Diviner"],
    bff: ["Orochi", "Rinkah", "Setsuna", "Peri"],
    sRank: ["Corrin (M)", "Jakob", "Silas", "Kaze", "Ryoma", "Takumi", "Saizo", "Kaden", "Hinata", "Azama", "Subaki", "Hayato", "Odin", "Arthur"],
    strMod: 3,
    magMod: 0,
    sklMod: -1,
    spdMod: -1,
    lckMod: 0,
    defMod: -1,
    resMod: 1,
    hpGR: 30,
    strGR: 65,
    magGR: 0,
    sklGR: 20,
    spdGR: 50,
    lckGR: 30,
    defGR: 25,
    resGR: 40,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 1],
    apb: [1, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 1, 0, 0, 1],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var reina = {
    n: "Reina",
    vName: "reina",
    baseClass: ["Sky Knight", "Diviner"],
    bff: [],
    sRank: ["Corrin (M)"],
    strMod: 2,
    magMod: 0,
    sklMod: 0,
    spdMod: 2,
    lckMod: -1,
    defMod: -2,
    resMod: -1,
    hpGR: 40,
    strGR: 45,
    magGR: 5,
    sklGR: 20,
    spdGR: 45,
    lckGR: 10,
    defGR: 20,
    resGR: 10,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 1, 0, 0, 0],
    spb: [1, 0, 0, 1, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var kaden = {
    n: "Kaden",
    vName: "kaden",
    baseClass: ["Kitsune",  "Diviner"],
    bff: ["Hinata", "Azama", "Hayato", "Keaton"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Hinoka", "Sakura", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro", "Charlotte", "Peri"],
    strMod: 1,
    magMod: 0,
    sklMod: -3,
    spdMod: 2,
    lckMod: 1,
    defMod: -2,
    resMod: 2,
    hpGR: 45,
    strGR: 40,
    magGR: 10,
    sklGR: 25,
    spdGR: 45,
    lckGR: 50,
    defGR: 35,
    resGR: 40,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 1],
    apb: [0, 0, 0, 0, 1, 0, 0],
    spb: [0, 0, 0, 1, 0, 0, 1],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var ryoma = {
    n: "Ryoma",
    vName: "ryoma",
    baseClass: ["Samurai", "Sky Knight"],
    bff: ["Saizo", "Silas", "Takumi", "Xander"],
    sRank: ["Corrin (F)", "Felicia", "Azura", "Mozu", "Rinkah", "Orochi", "Kagero", "Hana", "Setsuna", "Oboro", "Camilla", "Elise"],
    strMod: 1,
    magMod: 0,
    sklMod: 2,
    spdMod: 1,
    lckMod: 1,
    defMod: -2,
    resMod: -2,
    hpGR: 50,
    strGR: 45,
    magGR: 0,
    sklGR: 50,
    spdGR: 45,
    lckGR: 40,
    defGR: 35,
    resGR: 25,
    cpb: [0, 0, 0, 1, 0, 0, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 1, 0, 0, 0, 0],
    spb: [0, 0, 0, 2, 0, 0, 0],
    isChild: false,
    isRoyal: true,
    sex: "M"
}
var scarlet = {
    n: "Scarlet",
    vName: "scarlet",
    baseClass: ["Wyvern Rider", "Outlaw", "Knight"],
    bff: [],
    sRank: ["Corrin (M)"],
    strMod: 2,
    magMod: 0,
    sklMod: 0,
    spdMod: 1,
    lckMod: -1,
    defMod: 0,
    resMod: -2,
    hpGR: 30,
    strGR: 45,
    magGR: 20,
    sklGR: 40,
    spdGR: 50,
    lckGR: 40,
    defGR: 25,
    resGR: 20,
    cpb: [1, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 1, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 1, 0],
    spb: [1, 0, 1, 0, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var yukimura = {
    n: "Yukimura",
    vName: "yukimura",
    baseClass: ["Apothecary", "Samurai", "Monk"],
    bff: [],
    sRank: ["Corrin (F)"],
    strMod: -1,
    magMod: 0,
    sklMod: 3,
    spdMod: -1,
    lckMod: 0,
    defMod: -1,
    resMod: 0,
    hpGR: 25,
    strGR: 25,
    magGR: 5,
    sklGR: 40,
    spdGR: 15,
    lckGR: 30,
    defGR: 25,
    resGR: 30,
    cpb: [0, 0, 1, 0, 0, 0, 0],
    bpb: [0, 0, 1, 0, 0, 0, 0],
    apb: [1, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 1, 1, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var fuga = {
    n: "Fuga",
    vName: "fuga",
    baseClass: ["Samurai", "Oni Savage", "Monk"],
    bff: ["Hayato"],
    sRank: ["Corrin (F)"],
    strMod: 2,
    magMod: -1,
    sklMod: 1,
    spdMod: 0,
    lckMod: -1,
    defMod: 2,
    resMod: -2,
    hpGR: 20,
    strGR: 20,
    magGR: 0,
    sklGR: 15,
    spdGR: 5,
    lckGR: 20,
    defGR: 10,
    resGR: 10,
    cpb: [0, 0, 0, 0, 0, 1, 0],
    bpb: [1, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 1, 0, 0, 0, 0],
    spb: [1, 0, 0, 0, 0, 1, 0],
    isChild: false,
    isRoyal: false,
    sex: "M"
}
var anna = {
    n: "Anna",
    vName: "anna",
    baseClass: ["Outlaw", "Troubadour", "Apothecary"],
    bff: [],
    sRank: ["Corrin (M)"],
    strMod: -1,
    magMod: 1,
    sklMod: 0,
    spdMod: -1,
    lckMod: 2,
    defMod: -2,
    resMod: 2,
    hpGR: 35,
    strGR: 30,
    magGR: 55,
    sklGR: 30,
    spdGR: 40,
    lckGR: 70,
    defGR: 20,
    resGR: 45,
    cpb: [0, 0, 0, 0, 1, 0, 0],
    bpb: [0, 1, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 1, 0, 0],
    spb: [0, 0, 0, 0, 1, 0, 1],
    isChild: false,
    isRoyal: false,
    sex: "F"
}
var noPar = {
    n: "-",
    baseClass: ["-"],
    bff: [],
    sRank: [],
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    isChild: false,
    isRoyal: false,
    sex: "A"
}
var kanaF = {
    n: "Kana (F)",
    vName: "kanaF",
    baseClass: ["Nohr Princess"],
    firstParent: corrinM,
    secondParent: ["-", "Felicia", "Azura", "Mozu", "Elise", "Effie", "Nyx", "Camilla", "Selena", "Beruka", "Peri", "Charlotte", "Flora", "Rinkah", "Sakura", "Hana", "Hinoka", "Orochi", "Setsuna", "Oboro", "Kagero", "Reina", "Scarlet", "Anna", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Selkie", "Mitama", "Caeldori", "Rhajat", "Jakob", "Silas", "Kaze", "Shura", "Ryoma", "Takumi", "Saizo", "Azama", "Hinata", "Subaki", "Hayato", "Kaden", "Xander", "Leo", "Laslow", "Odin", "Niles", "Arthur", "Benny", "Keaton", "Gunter", "Yukimura", "Izana", "Fuga", "Siegbert", "Forrest", "Ignatius", "Percy", "Dwyer", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame"],
    bff: ["Midori", "Velouria", "Selkie", "Mitama"],
    sRank: ["Shigure", "Dwyer", "Siegbert", "Forrest", "Ignatius", "Percy", "Shiro", "Kiragi", "Asugi", "Hisame"],
    baseHPGR: 30,
    baseStrGR: 35,
    baseMagGR: 30,
    baseSklGR: 40,
    baseSpdGR: 45,
    baseLckGR: 45,
    baseDefGR: 25,
    baseResGR: 25,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 30,
    strGR: 35,
    magGR: 30,
    sklGR: 40,
    spdGR: 45,
    lckGR: 45,
    defGR: 25,
    resGR: 25,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: true
}
var kanaM = {
    n: "Kana (M)",
    vName: "kanaM",
    baseClass: ["Nohr Prince"],
    firstParent: corrinF,
    secondParent: ["-", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Arthur", "Odin", "Niles", "Laslow", "Benny", "Leo", "Keaton", "Xander", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Fuga", "Siegbert", "Forrest", "Ignatius", "Percy", "Dwyer", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Flora", "Felicia", "Azura", "Mozu", "Elise", "Effie", "Nyx", "Camilla", "Selena", "Beruka", "Peri", "Charlotte", "Flora", "Rinkah", "Sakura", "Hana", "Hinoka", "Orochi", "Setsuna", "Oboro", "Kagero", "Reina", "Scarlet", "Anna", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Selkie", "Mitama", "Caeldori", "Rhajat"],
    bff: ["Siegbert", "Percy", "Shiro"],
    sRank: ["Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Selkie", "Mitama", "Caeldori", "Rhajat"],
    baseHPGR: 30,
    baseStrGR: 35,
    baseMagGR: 30,
    baseSklGR: 40,
    baseSpdGR: 45,
    baseLckGR: 45,
    baseDefGR: 25,
    baseResGR: 25,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 30,
    strGR: 35,
    magGR: 30,
    sklGR: 40,
    spdGR: 45,
    lckGR: 45,
    defGR: 25,
    resGR: 25,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: true, 
    lr: ""
}
var shigure = {
    n: "Shigure",
    vName: "shigure",
    baseClass: ["Sky Knight", "Troubadour"],
    firstParent: azura,
    secondParent: ["-", "Gunter", "Izana", "Arthur", "Azama", "Benny", "Corrin (M)", "Hayato", "Hinata", "Jakob", "Kaden", "Kaze", "Keaton", "Laslow", "Leo", "Niles", "Odin", "Ryoma", "Saizo", "Silas", "Subaki", "Takumi", "Xander", "Yukimura", "Fuga", "Kana (M)", "Siegbert", "Forrest", "Ignatius", "Percy", "Dwyer", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Corrin (F)", "Felicia", "Azura", "Mozu", "Elise", "Effie", "Nyx", "Camilla", "Selena", "Beruka", "Peri", "Charlotte", "Flora", "Rinkah", "Sakura", "Hana", "Hinoka", "Orochi", "Setsuna", "Oboro", "Kagero", "Reina", "Scarlet", "Anna", "Kana (F)", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Selkie", "Mitama", "Caeldori", "Rhajat"],
    bff: ["Forrest", "Hisame"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Selkie", "Mitama", "Caeldori", "Rhajat"],
    baseHPGR: 35,
    baseStrGR: 45,
    baseMagGR: 5,
    baseSklGR: 45,
    baseSpdGR: 35,
    baseLckGR: 25,
    baseDefGR: 35,
    baseResGR: 25,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 35,
    strGR: 45,
    magGR: 5,
    sklGR: 45,
    spdGR: 35,
    lckGR: 25,
    defGR: 35,
    resGR: 25,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: true, 
    lr: ""
}
var dwyer = {
    n: "Dwyer",
    vName: "dwyer",
    baseClass: ["Troubadour", "Cavalier"],
    firstParent: jakob,
    secondParent: ["-", "Azura", "Beruka", "Camilla", "Charlotte", "Corrin (F)", "Effie", "Elise", "Felicia", "Hana", "Hinoka", "Kagero", "Mozu", "Nyx", "Oboro", "Orochi", "Peri", "Rinkah", "Selena", "Sakura", "Setsuna", "Corrin (M)", "Gunter", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Ignatius", "Percy", "Flora", "Reina", "Scarlet", "Anna", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Arthur", "Odin", "Niles", "Laslow", "Benny", "Leo", "Keaton", "Xander", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura"],
    bff: ["Percy", "Asugi", "Kiragi"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Selkie", "Mitama", "Caeldori", "Rhajat"],
    baseHPGR: 45,
    baseStrGR: 45,
    baseMagGR: 30,
    baseSklGR: 20,
    baseSpdGR: 30,
    baseLckGR: 30,
    baseDefGR: 30,
    baseResGR: 35,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 45,
    strGR: 45,
    magGR: 30,
    sklGR: 20,
    spdGR: 30,
    lckGR: 30,
    defGR: 30,
    resGR: 35,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var sophie = {
    n: "Sophie",
    vName: "sophie",
    baseClass: ["Cavalier", "Mercenary"],
    firstParent: silas,
    secondParent: ["-", "Azura", "Beruka", "Camilla", "Charlotte", "Corrin (F)", "Effie", "Elise", "Felicia", "Hana", "Hinoka", "Kagero", "Mozu", "Nyx", "Oboro", "Orochi", "Peri", "Rinkah", "Selena", "Sakura", "Setsuna", "Corrin (M)", "Gunter", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Ignatius", "Percy", "Flora", "Reina", "Scarlet", "Anna", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Arthur", "Odin", "Niles", "Laslow", "Benny", "Leo", "Keaton", "Xander", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura"],
    bff: ["Velouria", "Soleil", "Caeldori"],
    sRank: ["Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Siegbert", "Forrest", "Ignatius", "Percy", "Shiro", "Kiragi", "Asugi", "Hisame"],
    baseHPGR: 35,
    baseStrGR: 35,
    baseMagGR: 10,
    baseSklGR: 55,
    baseSpdGR: 50,
    baseLckGR: 35,
    baseDefGR: 25,
    baseResGR: 35,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 35,
    strGR: 35,
    magGR: 10,
    sklGR: 55,
    spdGR: 50,
    lckGR: 35,
    defGR: 25,
    resGR: 35,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var midori = {
    n: "Midori",
    vName: "midori",
    baseClass: ["Apothecary", "Ninja"],
    firstParent: kaze,
    secondParent: ["-", "Azura", "Beruka", "Camilla", "Charlotte", "Corrin (F)", "Effie", "Elise", "Felicia", "Hana", "Hinoka", "Kagero", "Mozu", "Nyx", "Oboro", "Orochi", "Peri", "Rinkah", "Selena", "Sakura", "Setsuna", "Corrin (M)", "Gunter", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Ignatius", "Percy", "Flora", "Reina", "Scarlet", "Anna", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Arthur", "Odin", "Niles", "Laslow", "Benny", "Leo", "Keaton", "Xander", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura"],
    bff: ["Kana (F)", "Ophelia", "Selkie"],
    sRank: ["Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Siegbert", "Forrest", "Ignatius", "Percy", "Shiro", "Kiragi", "Asugi", "Hisame"],
    baseHPGR: 45,
    baseStrGR: 35,
    baseMagGR: 5,
    baseSklGR: 55,
    baseSpdGR: 35,
    baseLckGR: 50,
    baseDefGR: 30,
    baseResGR: 20,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 45,
    strGR: 35,
    magGR: 5,
    sklGR: 55,
    spdGR: 35,
    lckGR: 50,
    defGR: 30,
    resGR: 20,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: "" 
}
var siegbert = {
    n: "Siegbert",
    vName: "siegbert",
    baseClass: ["Cavalier", "Wyvern Rider"],
    firstParent: xander,
    secondParent: ["-", "Azura", "Beruka", "Charlotte", "Corrin (F)", "Effie", "Felicia", "Hinoka", "Mozu", "Nyx", "Peri", "Sakura", "Selena", "Corrin (M)", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Arthur", "Odin", "Niles", "Laslow", "Benny", "Leo", "Keaton", "Rinkah", "Hana", "Orochi", "Setsuna", "Oboro", "Kagero", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Forrest", "Ignatius", "Percy", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina"],
    bff: ["Kana (M)", "Forrest", "Ignatius", "Shiro"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Mitama", "Caeldori"],
    baseHPGR: 40,
    baseStrGR: 45,
    baseMagGR: 5,
    baseSklGR: 45,
    baseSpdGR: 45,
    baseLckGR: 45,
    baseDefGR: 35,
    baseResGR: 20,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 40,
    strGR: 45,
    magGR: 5,
    sklGR: 45,
    spdGR: 45,
    lckGR: 45,
    defGR: 35,
    resGR: 20,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: true, 
    lr: ""
}
var forrest = {
    n: "Forrest",
    vName: "forrest",
    baseClass: ["Troubadour", "Dark Mage"],
    firstParent: leo,
    secondParent: ["-", "Azura", "Beruka", "Charlotte", "Corrin (F)", "Effie", "Felicia", "Hinoka", "Mozu", "Nyx", "Peri", "Sakura", "Selena", "Corrin (M)", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Arthur", "Odin", "Niles", "Laslow", "Benny", "Xander", "Keaton", "Rinkah", "Hana", "Orochi", "Setsuna", "Oboro", "Kagero", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Ignatius", "Percy", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina"],
    bff: ["Shigure", "Siegbert", "Ignatius", "Kiragi"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Selkie", "Rhajat"],
    baseHPGR: 55,
    baseStrGR: 15,
    baseMagGR: 65,
    baseSklGR: 20,
    baseSpdGR: 35,
    baseLckGR: 25,
    baseDefGR: 25,
    baseResGR: 55,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 55,
    strGR: 15,
    magGR: 65,
    sklGR: 20,
    spdGR: 35,
    lckGR: 25,
    defGR: 25,
    resGR: 55,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: true, 
    lr: ""
}
var ignatius = {
    n: "Ignatius",
    vName: "ignatius",
    baseClass: ["Knight", "Fighter"],
    firstParent: benny,
    secondParent: ["-", "Azura", "Beruka", "Camilla", "Charlotte", "Corrin (F)", "Effie", "Elise", "Felicia", "Mozu", "Nyx", "Oboro", "Peri", "Rinkah", "Selena", "Corrin (M)", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Arthur", "Odin", "Niles", "Laslow", "Xander", "Leo", "Keaton", "Sakura", "Hana", "Orochi", "Setsuna", "Hinoka", "Kagero", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Percy", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina"],
    bff: ["Siegbert", "Forrest", "Percy", "Asugi"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Selkie", "Caeldori"],
    baseHPGR: 40,
    baseStrGR: 50,
    baseMagGR: 0,
    baseSklGR: 40,
    baseSpdGR: 30,
    baseLckGR: 55,
    baseDefGR: 45,
    baseResGR: 35,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 40,
    strGR: 50,
    magGR: 0,
    sklGR: 40,
    spdGR: 30,
    lckGR: 55,
    defGR: 45,
    resGR: 35,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var velouria = {
    n: "Velouria",
    vName: "velouria",
    baseClass: ["Wolfskin", "Fighter"],
    firstParent: keaton,
    secondParent: ["-", "Azura", "Beruka", "Camilla", "Charlotte", "Corrin (F)", "Effie", "Elise", "Felicia", "Hana", "Mozu", "Nyx", "Peri", "Rinkah", "Selena", "Corrin (M)", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Arthur", "Odin", "Niles", "Laslow", "Benny", "Leo", "Xander", "Sakura", "Hinoka", "Orochi", "Setsuna", "Oboro", "Kagero", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Ignatius", "Percy", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Ophelia", "Soleil", "Nina"],
    bff: ["Kana (F)", "Sophie", "Nina", "Selkie"],
    sRank: ["Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Siegbert", "Forrest", "Ignatius", "Percy", "Hisame", "Kiragi"],
    baseHPGR: 50,
    baseStrGR: 50,
    baseMagGR: 0,
    baseSklGR: 40,
    baseSpdGR: 40,
    baseLckGR: 35,
    baseDefGR: 45,
    baseResGR: 30,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 50,
    strGR: 50,
    magGR: 0,
    sklGR: 40,
    spdGR: 40,
    lckGR: 35,
    defGR: 45,
    resGR: 30,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var percy = {
    n: "Percy",
    vName: "percy",
    baseClass: ["Wyvern Rider", "Fighter"],
    firstParent: arthur,
    secondParent: ["-", "Azura", "Beruka", "Camilla", "Charlotte", "Corrin (F)", "Effie", "Elise", "Felicia", "Kagero", "Mozu", "Nyx", "Peri", "Selena", "Setsuna", "Corrin (M)", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Xander", "Odin", "Niles", "Laslow", "Benny", "Leo", "Keaton", "Rinkah", "Hana", "Orochi", "Sakura", "Oboro", "Hinoka", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Ignatius", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina"],
    bff: ["Kana (M)", "Dwyer", "Ignatius", "Hisame"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina", "Mitama", "Rhajat"],
    baseHPGR: 30,
    baseStrGR: 30,
    baseMagGR: 5,
    baseSklGR: 45,
    baseSpdGR: 40,
    baseLckGR: 75,
    baseDefGR: 55,
    baseResGR: 15,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 30,
    strGR: 30,
    magGR: 5,
    sklGR: 45,
    spdGR: 40,
    lckGR: 75,
    defGR: 55,
    resGR: 15,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var ophelia = {
    n: "Ophelia",
    vName: "ophelia",
    baseClass: ["Dark Mage", "Samurai"],
    firstParent: odin,
    secondParent: ["-", "Azura", "Beruka", "Camilla", "Charlotte", "Corrin (F)", "Effie", "Elise", "Felicia", "Kagero", "Mozu", "Nyx", "Orochi", "Peri", "Selena", "Corrin (M)", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Arthur", "Xander", "Niles", "Laslow", "Benny", "Leo", "Keaton", "Rinkah", "Hana", "Sakura", "Setsuna", "Oboro", "Hinoka", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Ignatius", "Percy", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Soleil", "Nina"],
    bff: ["Midori", "Soleil", "Rhajat"],
    sRank: ["Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Siegbert", "Forrest", "Ignatius", "Percy", "Shiro", "Hisama"],
    baseHPGR: 45,
    baseStrGR: 15,
    baseMagGR: 45,
    baseSklGR: 40,
    baseSpdGR: 45,
    baseLckGR: 65,
    baseDefGR: 20,
    baseResGR: 30,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 45,
    strGR: 15,
    magGR: 45,
    sklGR: 40,
    spdGR: 45,
    lckGR: 65,
    defGR: 20,
    resGR: 30,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: "" 
}
var soleil = {
    n: "Soleil",
    vName: "soleil",
    baseClass: ["Mercenary", "Ninja"],
    firstParent: laslow,
    secondParent: ["-", "Azura", "Beruka", "Camilla", "Charlotte", "Corrin (F)", "Effie", "Elise", "Felicia", "Hana", "Mozu", "Nyx", "Orochi", "Peri", "Selena", "Corrin (M)", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Arthur", "Odin", "Niles", "Xander", "Benny", "Leo", "Keaton", "Rinkah", "Sakura", "Hinoka", "Setsuna", "Oboro", "Kagero", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Ignatius", "Percy", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Nina"],
    bff: ["Sophie", "Ophelia", "Nina", "Mitama"],
    sRank: ["Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Siegbert", "Forrest", "Ignatius", "Percy", "Kiragi", "Asugi"],
    baseHPGR: 25,
    baseStrGR: 60,
    baseMagGR: 0,
    baseSklGR: 35,
    baseSpdGR: 35,
    baseLckGR: 45,
    baseDefGR: 35,
    baseResGR: 40,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 25,
    strGR: 60,
    magGR: 0,
    sklGR: 35,
    spdGR: 35,
    lckGR: 45,
    defGR: 35,
    resGR: 40,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: "" 
}
var nina = {
    n: "Nina",
    vName: "nina",
    baseClass: ["Outlaw", "Dark Mage"],
    firstParent: niles,
    secondParent: ["-", "Azura", "Beruka", "Camilla", "Charlotte", "Corrin (F)", "Effie", "Elise", "Felicia", "Mozu", "Nyx", "Oboro", "Peri", "Selena", "Setsuna", "Corrin (M)", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Arthur", "Odin", "Xander", "Laslow", "Benny", "Leo", "Keaton", "Rinkah", "Hana", "Orochi", "Sakura", "Hinoka", "Kagero", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Kana (M)", "Shigure", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Ignatius", "Percy", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil"],
    bff: ["Soleil", "Velouria", "Caeldori"],
    sRank: ["Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Siegbert", "Forrest", "Ignatius", "Percy", "Asugi", "Shiro"],
    baseHPGR: 30,
    baseStrGR: 45,
    baseMagGR: 30,
    baseSklGR: 35,
    baseSpdGR: 40,
    baseLckGR: 50,
    baseDefGR: 25,
    baseResGR: 45,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 30,
    strGR: 45,
    magGR: 30,
    sklGR: 35,
    spdGR: 40,
    lckGR: 50,
    defGR: 25,
    resGR: 45,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var shiro = {
    n: "Shiro",
    vName: "shiro",
    baseClass: ["Spear Fighter", "Samurai"],
    firstParent: ryoma,
    secondParent: ["-", "Azura", "Camilla", "Corrin (F)", "Elise", "Felicia", "Hana", "Kagero", "Mozu", "Oboro", "Orochi", "Rinkah", "Setsuna", "Corrin (M)", "Gunter", "Jakob", "Kaze", "Silas", "Shura", "Izana", "Fuga", "Elise", "Effie", "Nyx", "Camilla", "Selena", "Beruka", "Peri", "Charlotte", "Arthur", "Odin", "Niles", "Laslow", "Benny", "Leo", "Keaton", "Xander", "Subaki", "Saizo", "Azama", "Hayato", "Hinata", "Takumi", "Kaden", "Ryoma", "Yukimura", "Kana (M)", "Shigure", "Kiragi", "Asugi", "Hisame", "Siegbert", "Forrest", "Ignatius", "Percy", "Kana (F)", "Selkie", "Mitama", "Caeldori", "Rhajat", "Sophie", "Midori", "Velouria", "Ophelia", "Soleil", "Nina"],
    bff: ["Kana (M)", "Kiragi", "Asugi", "Siegbert"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Selkie", "Mitama", "Caeldori", "Rhajat", "Ophelia", "Nina"],
    baseHPGR: 50,
    baseStrGR: 50,
    baseMagGR: 0,
    baseSklGR: 40,
    baseSpdGR: 35,
    baseLckGR: 35,
    baseDefGR: 45,
    baseResGR: 30,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 50,
    strGR: 50,
    magGR: 0,
    sklGR: 40,
    spdGR: 35,
    lckGR: 35,
    defGR: 45,
    resGR: 30,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: true, 
    lr: ""
}
var kiragi = {
    n: "Kiragi",
    vName: "kiragi",
    baseClass: ["Archer", "Spear Fighter"],
    firstParent: takumi,
    secondParent: ["-", "Azura", "Camilla", "Corrin (F)", "Elise", "Felicia", "Hana", "Kagero", "Mozu", "Oboro", "Orochi", "Rinkah", "Setsuna"],
    bff: ["Dwyer", "Shiro", "Hisame", "Forrest"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Selkie", "Mitama", "Caeldori", "Rhajat", "Velouria", "Soleil"],
    baseHPGR: 45,
    baseStrGR: 40,
    baseMagGR: 0,
    baseSklGR: 45,
    baseSpdGR: 50,
    baseLckGR: 45,
    baseDefGR: 40,
    baseResGR: 15,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 45,
    strGR: 40,
    magGR: 0,
    sklGR: 45,
    spdGR: 50,
    lckGR: 45,
    defGR: 40,
    resGR: 15,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: true, 
    lr: ""
}
var asugi = {
    n: "Asugi",
    vName: "asugi",
    baseClass: ["Ninja", "Samurai"],
    firstParent: saizo,
    secondParent: ["-", "Azura", "Beruka", "Charlotte", "Corrin (F)", "Felicia", "Hana", "Hinoka", "Kagero", "Mozu", "Oboro", "Orochi", "Rinkah", "Sakura", "Setsuna"],
    bff: ["Dwyer", "Shiro", "Hisame", "Ignatius"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Selkie", "Mitama", "Caeldori", "Rhajat", "Soleil", "Nina"],
    baseHPGR: 40,
    baseStrGR: 45,
    baseMagGR: 50,
    baseSklGR: 55,
    baseSpdGR: 45,
    baseLckGR: 50,
    baseDefGR: 30,
    baseResGR: 20,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 40,
    strGR: 45,
    magGR: 50,
    sklGR: 55,
    spdGR: 45,
    lckGR: 50,
    defGR: 30,
    resGR: 20,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: false, 
    lr: "" 
}
var selkie = {
    n: "Selkie",
    vName: "selkie",
    baseClass: ["Kitsune", "Diviner"],
    firstParent: kaden,
    secondParent: ["-", "Azura", "Charlotte", "Corrin (F)", "Felicia", "Hana", "Hinoka", "Kagero", "Mozu", "Oboro", "Orochi", "Peri", "Rinkah", "Sakura", "Setsuna"],
    bff: ["Kana (F)", "Midori", "Rhajat", "Velouria"],
    sRank: ["Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Shiro", "Kiragi", "Asugi", "Hisame", "Ignatius", "Forrest"],
    baseHPGR: 35,
    baseStrGR: 30,
    baseMagGR: 15,
    baseSklGR: 35,
    baseSpdGR: 55,
    baseLckGR: 60,
    baseDefGR: 30,
    baseResGR: 50,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 35,
    strGR: 30,
    magGR: 15,
    sklGR: 35,
    spdGR: 55,
    lckGR: 60,
    defGR: 30,
    resGR: 50,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var hisame = {
    n: "Hisame",
    vName: "hisame",
    baseClass: ["Samurai", "Oni Savage"],
    firstParent: hinata,
    secondParent: ["-", "Azura", "Corrin (F)", "Felicia", "Hana", "Hinoka", "Kagero", "Mozu", "Oboro", "Orochi", "Peri", "Rinkah", "Sakura", "Selena", "Setsuna"],
    bff: ["Shigure", "Kiragi", "Asugi", "Percy"],
    sRank: ["Corrin (F)", "Kana (F)", "Sophie", "Midori", "Selkie", "Mitama", "Caeldori", "Rhajat", "Velouria", "Ophelia"],
    baseHPGR: 50,
    baseStrGR: 40,
    baseMagGR: 0,
    baseSklGR: 40,
    baseSpdGR: 40,
    baseLckGR: 25,
    baseDefGR: 30,
    baseResGR: 20,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 50,
    strGR: 40,
    magGR: 0,
    sklGR: 40,
    spdGR: 40,
    lckGR: 25,
    defGR: 30,
    resGR: 20,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "M",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var mitama = {
    n: "Mitama",
    vName: "mitama",
    baseClass: ["Shrine Maiden", "Apothecary"],
    firstParent: azama,
    secondParent: ["-", "Azura", "Beruka", "Corrin (F)", "Effie", "Felicia", "Hana", "Hinoka", "Kagero", "Mozu", "Oboro", "Orochi", "Rinkah", "Sakura", "Setsuna"],
    bff: ["Kana (F)", "Caeldori", "Rhajat", "Soleil"],
    sRank: ["Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Shiro", "Kiragi", "Asugi", "Hisame", "Percy", "Siegbert"],
    baseHPGR: 45,
    baseStrGR: 40,
    baseMagGR: 35,
    baseSklGR: 45,
    baseSpdGR: 50,
    baseLckGR: 50,
    baseDefGR: 30,
    baseResGR: 20,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 45,
    strGR: 40,
    magGR: 35,
    sklGR: 45,
    spdGR: 50,
    lckGR: 50,
    defGR: 30,
    resGR: 20,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var caeldori = {
    n: "Caeldori",
    vName: "caeldori",
    baseClass: ["Sky Knight", "Samurai"],
    firstParent: subaki,
    secondParent: ["-", "Azura", "Corrin (F)", "Felicia", "Hana", "Hinoka", "Kagero", "Mozu", "Nyx", "Oboro", "Orochi", "Rinkah", "Sakura", "Selena", "Setsuna"],
    bff: ["Sophie", "Mitama", "Rhajat", "Nina"],
    sRank: ["Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Shiro", "Kiragi", "Asugi", "Hisame", "Siegbert", "Ignatius"],
    baseHPGR: 55,
    baseStrGR: 35,
    baseMagGR: 15,
    baseSklGR: 40,
    baseSpdGR: 40,
    baseLckGR: 45,
    baseDefGR: 35,
    baseResGR: 20,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 55,
    strGR: 35,
    magGR: 15,
    sklGR: 40,
    spdGR: 40,
    lckGR: 45,
    defGR: 35,
    resGR: 20,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var rhajat = {
    n: "Rhajat",
    vName: "rhajat",
    baseClass: ["Diviner", "Oni Savage"],
    firstParent: hayato,
    secondParent: ["-", "Azura", "Corrin (F)", "Effie", "Felicia", "Hana", "Hinoka", "Kagero", "Mozu", "Nyx", "Oboro", "Orochi", "Rinkah", "Sakura", "Setsuna"],
    bff: ["Caeldori", "Mitama", "Selkie", "Ophelia"],
    sRank: ["Corrin (F)", "Corrin (M)", "Kana (M)", "Shigure", "Dwyer", "Shiro", "Kiragi", "Asugi", "Hisame", "Forrest", "Percy"],
    baseHPGR: 40,
    baseStrGR: 15,
    baseMagGR: 60,
    baseSklGR: 10,
    baseSpdGR: 50,
    baseLckGR: 30,
    baseDefGR: 25,
    baseResGR: 35,
    strMod: 0,
    magMod: 0,
    sklMod: 0,
    spdMod: 0,
    lckMod: 0,
    defMod: 0,
    resMod: 0,
    hpGR: 40,
    strGR: 15,
    magGR: 60,
    sklGR: 10,
    spdGR: 50,
    lckGR: 30,
    defGR: 25,
    resGR: 35,
    cpb: [0, 0, 0, 0, 0, 0, 0],
    bpb: [0, 0, 0, 0, 0, 0, 0],
    apb: [0, 0, 0, 0, 0, 0, 0],
    spb: [0, 0, 0, 0, 0, 0, 0],
    sex: "F",
    isChild: true,
    isRoyal: false, 
    lr: ""
}
var allUnits = [corrinF, corrinM, gunter, felicia, jakob, kaze, azura, silas, shura, izana, mozu, elise, arthur, effie, odin, niles, nyx, camilla, selena, beruka, laslow, peri, benny, charlotte, leo, keaton, xander, flora, kanaF, kanaM, dwyer, shigure, sophie, midori, siegbert, forrest, ignatius, velouria, percy, ophelia, soleil, nina, rinkah, sakura, hana, subaki, saizo, orochi, hinoka, azama, setsuna, hayato, oboro, hinata, takumi, kagero, reina, kaden, ryoma, scarlet, yukimura, shiro, kiragi, asugi, selkie, hisame, mitama, caeldori, rhajat, fuga, anna]
var allParents = [corrinF, corrinM, gunter, felicia, jakob, kaze, azura, silas, shura, izana, mozu, elise, arthur, effie, odin, niles, nyx, camilla, selena, beruka, laslow, peri, benny, charlotte, leo, keaton, xander, flora, anna, dwyer, shigure, sophie, midori, siegbert, forrest, ignatius, velouria, percy, ophelia, soleil, nina, rinkah, sakura, hana, subaki, saizo, orochi, hinoka, azama, setsuna, hayato, oboro, hinata, takumi, kagero, reina, kaden, ryoma, scarlet, yukimura, shiro, kiragi, asugi, selkie, hisame, mitama, caeldori, rhajat, fuga, anna];
var allFG = [corrinF, corrinM, gunter, felicia, jakob, kaze, azura, silas, shura, izana, mozu, elise, arthur, effie, odin, niles, nyx, camilla, selena, beruka, laslow, peri, benny, charlotte, leo, keaton, xander, flora, rinkah, sakura, hana, subaki, saizo, orochi, hinoka, azama, setsuna, hayato, oboro, hinata, takumi, kagero, reina, kaden, ryoma, scarlet, yukimura, fuga, anna];
var allKiddies = [kanaF, kanaM, dwyer, shigure, sophie, midori, siegbert, forrest, ignatius, velouria, percy, ophelia, soleil, nina, shiro, kiragi, asugi, selkie, hisame, mitama, caeldori, rhajat];
var allC = [corrinF, corrinM, gunter, felicia, jakob, kaze, azura, silas, shura, izana, mozu, elise, arthur, effie, odin, niles, nyx, camilla, selena, beruka, laslow, peri, benny, charlotte, leo, keaton, xander, flora, anna, kanaF, kanaM, dwyer, shigure, sophie, midori, siegbert, forrest, ignatius, velouria, percy, ophelia, soleil, nina];
var allB = [corrinF, corrinM, gunter, felicia, jakob, kaze, azura, silas, shura, izana, mozu, rinkah, sakura, hana, subaki, saizo, orochi, hinoka, azama, setsuna, hayato, oboro, hinata, takumi, kagero, reina, kaden, ryoma, scarlet, yukimura, anna, kanaF, kanaM, dwyer, shigure, sophie, midori, shiro, kiragi, asugi, selkie, hisame, mitama, caeldori, rhajat];
var allR = [corrinF, corrinM, felicia, jakob, kaze, azura, silas, shura, mozu, elise, arthur, effie, odin, niles, nyx, camilla, selena, beruka, laslow, peri, benny, charlotte, leo, keaton, xander, flora, rinkah, sakura, hana, subaki, saizo, orochi, hinoka, azama, setsuna, hayato, oboro, hinata, takumi, kagero, reina, kaden, ryoma, scarlet, fuga, anna, kanaF, kanaM, dwyer, shigure, sophie, midori, siegbert, forrest, ignatius, velouria, percy, ophelia, soleil, nina, shiro, kiragi, asugi, selkie, hisame, mitama, caeldori, rhajat];
/* accepts unit object and returns array of unit's base growth rates */
function getGRBaseArr(unit) {
    return [unit.baseHPGR, unit.baseStrGR, unit.baseMagGR, unit.baseSklGR, unit.baseSpdGR, unit.baseLckGR, unit.baseDefGR, unit.baseResGR];
}
/* accepts unit object and returns array of unit's growth rates */
function getGRArrU(unit) {
    return [unit.hpGR, unit.strGR, unit.magGR, unit.sklGR, unit.spdGR, unit.lckGR, unit.defGR, unit.resGR];
}
